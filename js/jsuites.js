    ...
    obj.find = function(str) {
        ...
        if(str){
            ...
            for (var i = 0; i < obj.items.length; i++) {
                ...
                if (normalize) {
                    if (str == null || obj.items[i].classList.contains('jdropdown-selected') || normalize(obj.items[i].innerHTML.toLowerCase()).indexOf(str.toLowerCase()) != -1) {
                        obj.items[i].style.display = '';
                        visibleItems.push(obj.items[i])
                    } else {
