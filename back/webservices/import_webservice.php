...
class Import_webservice extends Webservice {
	...
	function saveClient() {
		...
		$clients =  json_decode(Post::input('clients'));
		...
		foreach ($clients as $key => $client) {
			...
			$c = Query::byColumn('client', 'client_code',  $client['COD_CLIENTE']);

		 	if ($c  == null) {

				$c = new Client();

				$c->seller_code = trim($client['COD_VENDEDOR']);
				$c->client_code = trim($client['COD_CLIENTE']);
				$c->nit = trim($client['NIT']);
				$c->name = trim($client['NOMBRE_CLIENTE']);
				$c->business_name = trim($client['NOMBRE_DE_NEGOCIO']);			
				// $c->typebusinnes = trim($client['TIPO_DE_NEGOCIO']);			
				$c->address = trim($client['DIRECCION']);			
				$c->city = trim($client['CIUDAD']);
				$c->neighborhood = trim($client['BARRIO']);
				$c->day_route = trim($client['DIA_VISITA']);
				$c->day = trim($client['DIA_LLAMADA']);			
				$c->state = trim($client['ESTADO']);
				$c->causal = trim($client['CAUSAL']);
			...
			if (isset($client['TELÉFONO'])) {

				$phones = explode(',', $client['TELÉFONO']);

				foreach ($phones  as $key => $phone) {

					$p = Query::byColumn('phone', 'phone',  $phone);
					if ($p  == null) {
						$p = new Phone();
	...
	function saveProducts() {
		...
		$products = Post::input('products');
		 ...
		 foreach ($products as $key => $product) {
		 	...
		 	$p = Query::byColumn('product', 'sap_code',  $product['COD_PRODUCTO']);

		 	if ($p  == null) {
		 		
		 	
			 	$p = new Product();
			 	$p->city = $product['CIUDAD'];
			 	$p->line = $product['LINEA'];
			 	$p->sap_code = $product['COD_PRODUCTO'];
				$p->category = $product['CATEGORIA'];
				$p->description = $product['DESCRIPCIÓN'];
				$p->measure = $product['MEDIDA'];
				$p->price = $product['PRECIO'];
				$p->iva = $product['IVA'];
				$p->state = $product['ESTADO'];
				$p->presentation = $product['PRESENTACIÓN'];
				$p->units = $product['UNIDAD'];
				$p->units_by_package = $product['UNIDADES POR PAQUETE'];
				$p->presentation_by_package = $product['PRESENTACIÓN POR PAQUETE'];
				$p->observation = $product['OBSERVACIÓN']; 			
				$p->typebusinnes = $product['TIPO DE NEGOCIO']; 			
