 ...
 Titan.modules.create({
	 ...
	 */
	 ready: function () {
	 	...
	 	//
			...
			if(location.host != 'dowesoft.com') {
				this.userLogin.val('edwinandeka@gmail.com')
				this.passLogin.val('eror1988eka')
			}
	 ...
	 */
	 initSession: function () {
	 	...
	 	r.done(function  (response) {
	 		if (response.StatusResult == 'OK') {
	 			...
	 			this.contract_end_date = moment(response.contractend_date).diff(moment(), 'days')

	 			this.contract_end_date += 1

	 			if (this.contract_end_date <= 5) {
	 				swal('Información', 'No fue posible procesar su pago, la suscripción se cancelará en ' + this.contract_end_date + ' días', 'error')
	 			}

	 			if (this.contract_end_date <= 0) {
	 				swal('Información', 'No fue posible procesar su pago, la suscripción fue cancelada.', 'error')
	 			} else {
	 ...
	 init: function (response) {
      ...
      __u__t_s = response.timmer_session;
